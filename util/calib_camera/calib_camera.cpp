...
static void calib(void)
{
    ...
    cv::calibrateCamera(cvarrToMat(objectPoints), cvarrToMat(imagePoints), cvSize(xsize,ysize),
                        cvarrToMat(intrinsics), cvarrToMat(distortionCoeff), cvarrToMat(rotationVectors), cvarrToMat(translationVectors), 0);
    ...
    for( k = 0; k < capturedImageNum; k++ ) {
        ...
        cv::Rodrigues( cvarrToMat(rotationVector), cvarrToMat(rotationMatrix) );
        for( j = 0; j < 3; j++ ) {
            for( i = 0; i < 3; i++ ) {
